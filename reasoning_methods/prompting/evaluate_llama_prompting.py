...
# Default settings for other hyperparameters remain:
MIN_NEW_TOKENS = 1
MAX_NEW_TOKENS = 256
TEMPERATURE = 0.7
SEED = 42
...
def main():
    # Detect hardware configuration and set hyperparameters accordingly
    global BATCH_SIZE, NUM_GPUS, MAX_MEMORY, DEVICE
    ...
    if args.dataset == "mmlu":
        # Add MMLU-specific argument
        parser.add_argument('--mmlu_subject', type=str, default='high_school_mathematics',
                          help='Subject area for MMLU dataset')
        args = parser.parse_args()
        
        # Update the split based on the subject argument
        dataset_config["split"] = args.mmlu_subject
